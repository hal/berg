...
describe("TESTS: Configuration => Datasource => MySQL (Finder)", () => {
    ...
    cy.startWildflyContainer().then((result) => {
      ...
      cy.executeInWildflyContainer(
        new AddModuleCommandBuilder()
          ...
          .withResource(
            "/home/fixtures/jdbc-drivers/mysql-connector-j-8.0.31.jar"
          )
  ...
  it("Should list created datasources in the column", () => {
    ...
    cy.get("#non-xa-dsc-" + mysqlDSToAdd.name.toLowerCase()).should(
      "be.visible"
    );
  ...
  it("View", () => {
    ...
    cy.url().should((url) => {
      expect(url).to.contain(
        "#data-source-configuration;name=" + mysqlDSToAdd.name
      );
    });
  });
  ...
  it("Disable", () => {
    ...
    cy.get(
      'span:contains("Data source ' + mysqlDSToAdd.name + ' disabled")'
    ).should("be.visible");
  });
