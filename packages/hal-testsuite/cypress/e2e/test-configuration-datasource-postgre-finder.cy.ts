import { AddModuleCommandBuilder,
  AddDataSourceBuilder,
  AddXADataSourceBuilder } from "@hal/commands"
...
describe('TESTS: Configuration => Datasource => PostgreSQL (Finder)', () => {
  ...
  before(() => {
    cy.task('start:postgres:container', {name: postgresContainerName, environmentProperties: {
  ...
  it('Test connection', () => {
    ...
    cy.get('span:contains("Successfully tested connection for data source ' + postgresDSToAdd.name + '")').should('be.visible')
  })
  ...
  it('Creates PostgreSQL datasource via Wizard', () => {
    ...
    cy.text('ds-configuration-driver-form', 'driver-module-name', postgresDriverModuleName)
    cy.get('.modal-footer .btn-primary').click()
  ...
  it('Creates PostgreSQL XA datasource via Wizard', () => {
    ...
    cy.get('.tag-manager-container .tm-tag').should('have.length', Object.entries(postgresDefaultXADS.xaProperties).length)
    cy.get('.modal-footer .btn-primary').click()
    cy.get('.wizard-pf-steps .wizard-pf-step.active .wizard-pf-step-number').should('have.text', '5')
    cy.get('.wizard-pf-steps .wizard-pf-step.active .wizard-pf-step-title').should('have.text', 'Connection')
    cy.text('ds-configuration-connection-form', 'user-name', postgresUser)
