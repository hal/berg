import { AddDataSourceBuilder, AddModuleCommandBuilder, AddXADataSourceBuilder } from "@hal/commands"
...
describe('TESTS: Configuration => Datasource',() => {
  ...
  const slqserverDefaultDS = {
    ...
    connectionUrl: 'jdbc:' + sqlserverDriverName + '://' + sqlserverContainerName 
  ...
  before(() => {
    cy.task('start:sqlserver:container', {name: 'sqlserver', environmentProperties: {
      'ACCEPT_EULA': 'Y',
    ...
    cy.startWildflyContainer().then((result) => {
      ...
      cy.executeInWildflyContainer(new AddModuleCommandBuilder()
      ...
      cy.task('execute:cli', {
        ...
        cy.executeInWildflyContainer(
          ...
          .build().toCLICommand())
        cy.executeInWildflyContainer(new AddXADataSourceBuilder()
  ...
  it('Should list created datasources in the column', () => {
    cy.navigateTo(managementEndpoint, 'configuration;path=configuration~subsystems!css~datasources!data-source-driver~datasources')
  ...
  it('Test connection', () => {
    ...
    cy.get('span:contains("Successfully tested connection for data source ' + sqlserverDSToAdd.name + '")').should('be.visible')
  })
  ...
  it('Creates SQLServer XA datasource via Wizard', () => {
    ...
    Object.entries(sqlserverDefaultXADS.xaProperties).forEach(([key, value]) => {
      cy.formInput('ds-configuration-properties-form', 'value').clear().type( key + '=' + value + '{enter}').trigger('change')
    })
    cy.get('.tag-manager-container .tm-tag').should('have.length', Object.entries(sqlserverDefaultXADS.xaProperties).length)
    cy.get('.modal-footer .btn-primary').click()
    cy.get('.wizard-pf-steps .wizard-pf-step.active .wizard-pf-step-number').should('have.text', '5')
    cy.get('.wizard-pf-steps .wizard-pf-step.active .wizard-pf-step-title').should('have.text', 'Connection')
    cy.text('ds-configuration-connection-form', 'user-name', sqlserverUser)
