...
describe("TESTS: Configuration => Datasource => SQL Server (Finder)", () => {
      ...
      .then(() => {
        cy.executeInWildflyContainer(
          new AddModuleCommandBuilder()
            ...
            .withResource(
              "/home/fixtures/jdbc-drivers/mssql-jdbc-11.2.1.jre11.jar"
            )
  ...
  it("Should list created datasources in the column", () => {
    ...
    cy.get("#non-xa-dsc-" + sqlserverDSToAdd.name.toLowerCase()).should(
      "be.visible"
    );
    cy.get("#xa-dsc-" + xaSqlserverDSToAdd.name.toLowerCase()).should(
      "be.visible"
    );
  });
  ...
  it("View", () => {
    ...
    cy.url().should((url) => {
      expect(url).to.contain(
        "#data-source-configuration;name=" + sqlserverDSToAdd.name
      );
    });
  });
  ...
  it("Disable", () => {
    ...
    cy.get(
      'span:contains("Data source ' + sqlserverDSToAdd.name + ' disabled")'
    ).should("be.visible");
  });
  ...
  it("Creates SQLServer XA datasource via Wizard", () => {
    ...
    Object.entries(sqlserverDefaultXADS.xaProperties).forEach(
      ([key, value]) => {
