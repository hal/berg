...
describe("TESTS: Configuration => Datasource => PostgreSQL (Finder)", () => {
  ...
  it("Should list created datasources in the column", () => {
    ...
    cy.get("#non-xa-dsc-" + postgresDSToAdd.name.toLowerCase()).should(
      "be.visible"
    );
    cy.get("#xa-dsc-" + xaPostgreDSToAdd.name.toLowerCase()).should(
      "be.visible"
    );
  });
  ...
  it("View", () => {
    ...
    cy.url().should((url) => {
      expect(url).to.contain(
        "#data-source-configuration;name=" + postgresDSToAdd.name
      );
    });
  });
  ...
  it("Disable", () => {
    ...
    cy.get(
      'span:contains("Data source ' + postgresDSToAdd.name + ' disabled")'
    ).should("be.visible");
  });
  ...
  it("Creates PostgreSQL datasource via Wizard", () => {
    ...
    cy.text(
      ...
      "connection-url",
      postgresDefaultDS.connectionUrl
    );
