export class AddXADataSource {
  ...
  toCLICommand() : string {
    Array.from(this.xaDataSourceProperties.entries()).map(([key, value]) => {
      return "{" + key + "=" + value + "}"
    }).join(',')
    return "\"xa-data-source add --name= " + this.name + 
    " --jndi-name=" + this.jndiName +  
    " --driver-name=" + this.driverName + 
    " --xa-datasource-class=" + this.xaDataSourceClass + 
    " --xa-datasource-properties={" + Array.from(this.xaDataSourceProperties).map(([key, value]) => {
      return key + "=>" + value
    }).join(',') + "}" +
